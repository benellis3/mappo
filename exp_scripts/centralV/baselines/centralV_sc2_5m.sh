...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../../.././docker.sh python3 /pymarl/src/main.py --exp_name="centralV/sc2_5m" with name=${TAG}__${filename}__${TIMESTAMP}__repeat${i} use_tensorboard=False  env_args.fully_observable=True &
  sleep 10s
done
