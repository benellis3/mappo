...
for j in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name="xxx_jakob_sc2_5m__lastshot_0_16" with xxx_logit_bias=0 name=${TAG}:0__xxx_jakob__sc2_5m__lastshot_0_16__${TIMESTAMP}__repeat${j} use_tensorboard=False &
  sleep 10s
done
