...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name="coma_wendelin_4agent_6x6" with name=${TAG}__coma_wendelin_4agent_6x6__${TIMESTAMP}__repeat${i} use_tensorboard=False &
  sleep 10s
done
