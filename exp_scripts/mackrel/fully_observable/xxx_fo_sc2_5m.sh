...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name="mackrel/jakob_sc2_5m" with multiagent_controller=mackrel_fo_mac name=${TAG}__xxx_fo_sc2_5m__${TIMESTAMP}__repeat${i} use_tensorboard=False &
  sleep 10s
done
