...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name=xxx_pp with debug_mode=level3_actions_only name=${TAG}__xxx_debug_level3only_pp_${TIMESTAMP}__repeat${i} use_tensorboard=False &
  sleep 10s
done
