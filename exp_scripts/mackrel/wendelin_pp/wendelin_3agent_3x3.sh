...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name="xxx_wendelin_3agent_3x3" with name=${TAG}__xxx_wendelin_3agent_3x3__${TIMESTAMP}__repeat${i} use_tensorboard=False &
  sleep 10s
done
