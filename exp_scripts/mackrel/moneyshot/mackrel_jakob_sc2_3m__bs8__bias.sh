...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../../.././docker.sh python3 /pymarl/src/main.py --exp_name="mackrel/moneyshot" with name=${TAG}__${filename}__${TIMESTAMP}__repeat${i} mackrel_logit_bias=1.5 use_tensorboard=False &
  sleep 10s
done
