...
for i in "${arr[@]}"; do
    for j in $(seq 1 $N_REPEAT); do
      ...
      NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name="xxx_jakob_sc2_3m" with xxx_logit_bias=0 name=${TAG}:${i}__xxx_jakob__sc2_3m__${TIMESTAMP}__repeat${j} use_tensorboard=False &
      sleep 10s
    done
done