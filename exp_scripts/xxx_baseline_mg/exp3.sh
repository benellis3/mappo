...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name="xxx_mg" with env_args.p_common=1 name=${TAG}__xxx_mg__${TIMESTAMP}__repeat${i} use_tensorboard=False &
  sleep 10s
done
