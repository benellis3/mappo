...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../../.././docker.sh python3 /pymarl/src/main.py --exp_name="flounderl/moneyshot" with name=${TAG}__${filename}__${TIMESTAMP}__repeat${i} use_tensorboard=False  env_args.fully_observable=False flounderl_fix_level1_pair=True flounderl_epsilon_start_level1=0.0 flounderl_delegate_if_zero_ck=True use_hdf_logger=True save_episode_samples=True save_model=True save_model_interval=100000 &
  sleep 10s
done
