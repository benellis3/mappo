...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../.././docker.sh CUDA_VISIBLE_DEVICES=" " python3 /pymarl/src/main.py --exp_name=coma_baseline_sc2_5m with name=${TAG}__coma_baseline_sc2_5m_${TIMESTAMP}__repeat${i}  use_tensorboard=False &
  sleep 10s
done
