...
do
    for i in $(seq 1 $N_REPEAT); do
      ...
      NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name="xxx_jakob_sc2_5m" with xxx_logit_bias=${i} name=${TAG}:$i__xxx_jakob__sc2_5m__${TIMESTAMP}__repeat${i} use_tensorboard=False &
      sleep 10s
    done
done
