...
for i in $(seq 1 $N_REPEAT); do
  ...
  NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name=iql_baseline_3m with name=${TAG}__iql_baseline_3m_${TIMESTAMP}__repeat${i} use_tensorboard=False &
  sleep 10s
done
