...
for b in "${arr[@]}"; do
    for i in $(seq 1 $N_REPEAT); do
      ...
      NV_GPU=${GPU_ID} ../.././docker.sh python3 /pymarl/src/main.py --exp_name="xxx_greg_boxpush" with xxx_logit_bias=${b} name=${TAG}:${b}__xxx_greg_boxpush__${TIMESTAMP}__repeat${i} use_tensorboard=False &
      sleep 10s
    done
done
