...
class CentralPPOLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        action_log_probs = th.nn.functional.log_softmax(action_logits, dim=-1)
        old_log_pac = th.gather(action_log_probs, dim=3, index=actions).squeeze(3).detach()
        ...
        for _ in range(0, self.mini_epochs_actor):
            ...
            entropy = th.sum( th.mean(th.sum(-1.0 * pacs * th.exp(pacs), dim=-1), dim=-1) * mask) / th.sum(mask)
