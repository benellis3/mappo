...
class COMAJointPolicyLoss(nn.Module):

    def __init__(self):
        super(COMAJointPolicyLoss, self).__init__()
    ...
    def forward(self, policies, advantages, actions, tformat):
        ...
        _act = _act.unsqueeze(0)
        try:
            _active_logits = th.gather(log_policies, _vdim(tformat), _act.long())
        except Exception as e:
            pass
        _active_logits[actions.unsqueeze(0) != actions.unsqueeze(0)] = 0.0 # mask logits for actions that are actually NaNs
        _adv[actions.unsqueeze(0) != actions.unsqueeze(0)] = 0.0
...
class COMAJointCriticLoss(nn.Module):

    def __init__(self):
        super(COMAJointCriticLoss, self).__init__()
...
class COMAJointLearner(BasicLearner):
    ...
    def __init__(self, multiagent_controller, logging_struct=None, args=None):
        ...
        self.critic_scheme_fn = lambda _agent_id1, _agent_id2: Scheme([dict(name="agent_id",
                                                                            rename="agent_ids",
                                                           ...
                                                           dict(name="actions",
                                                                ...
                                                                switch=self.args.joint_coma_critic_use_past_actions),
                                                           *[dict(name="actions__sample{}".format(0),
                                                                 # MIGHT BE PROBLEMATIC
                                                                 rename="other_agent_actions__sample{}".format(_i),
                                                                 transforms=[] if _agent_ids_2_pairing_id(
                                                                     (_agent_id1, _agent_id2),
                                                                     self.n_agents) != _i else [
                                                                     ("mask", dict(fill=0.0))],
                                                                 )
                                                             for _i in range(_n_agent_pairings(self.n_agents))],
                                                           dict(name="actions__sample{}".format(0),
                                                               # NEEDS TO BE SAMPLED IN
                                                               rename="past_action__sample{}".format(0),
                                                               transforms=[("shift", dict(steps=1)),
                                                                           # ("one_hot", dict(range=(0, self.n_actions - 1)))
                                                                           ],
                                                               ),
                                                            ...
                                                            *[dict(name="policies__sample{}".format(_i)) for _i in range(_n_agent_pair_samples(
                                                                self.n_agents))],  # select_agent_ids=[_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents)])
                                                            dict(name="state"),
        ...
        for _agent_id1, _agent_id2 in sorted(combinations(list(range(self.n_agents)), 2)):
            self.schemes["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))] = self.critic_scheme_fn(_agent_id1,_agent_id2)
            self.schemes["target_critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))] = self.critic_scheme_fn(_agent_id1,_agent_id2)
        ...
        self.input_columns = {}
        for _agent_id1, _agent_id2 in sorted(combinations(list(range(self.n_agents)), 2)):
            self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))] = {}
            self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]["avail_actions_id1"] = Scheme([dict(name="avail_actions", select_agent_ids=[_agent_id1])])
            self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]["avail_actions_id2"] = Scheme([dict(name="avail_actions", select_agent_ids=[_agent_id2])])
            self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]["qfunction"] = \
                Scheme([*[dict(name="other_agent_actions__sample{}".format(_i))
                          for _i in range(_n_agent_pairings(self.n_agents))],
                        dict(name="state"),
                        dict(name="past_action__sample{}".format(0)),
            ...
            self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]["observations"] = Scheme([dict(name="observations", select_agent_ids=[_agent_id1, _agent_id2])])
            self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]["avail_actions"] = Scheme([dict(name="avail_actions", select_agent_ids=[_agent_id1, _agent_id2])])
            self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]["agent_action"] = Scheme([dict(name="agent_action")])
            self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]["joint_policy"] = Scheme([dict(name="joint_policy")])
            # self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]["policies"] = Scheme([dict(name="policies__sample{}".format(_i)) for _i in range(_n_agent_pair_samples(self.n_agents))])
            ...
            self.input_columns["target_critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))] = self.input_columns["critic__agent{}".format(_agent_ids_2_pairing_id((_agent_id1, _agent_id2), self.n_agents))]
    ...
    def create_models(self, transition_scheme):
        ...
        self.critic = COMAJointCritic(input_shapes=self.input_shapes["critic__agent{}".format(0)], n_actions=self.n_actions, args=self.args)
    ...
    def train(self,
        ...
        def _optimize_critic(**kwargs):
            ...
            critic_loss, \
            critic_loss_tformat = COMAJointCriticLoss()(input=output_critic["qvalue"],
        ...
        policy_loss_function = partial(COMAJointPolicyLoss(),
