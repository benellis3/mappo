...
class PPOLearnerCentralV:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        entropy_mask = mask.squeeze().clone()
        ...
        norm_advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-5)

        # Calculate policy grad with mask
        pi_taken = th.gather(pi, dim=3, index=actions).squeeze(3)
        pi_taken[mask == 0] = 1e-10
        ...
        mb_step = len(mb_idx) // self.args.mini_epochs_num
        ts_step = len(ts_idx) // self.args.mini_batches_num

        log_pi_taken = log_pi_taken.detach()
        ...
        for i in range(0, self.args.mini_epochs_num):
            ...
            for j in range(0, self.args.mini_batches_num):
                ...
                mb_norm_adv = norm_advantages[curr_mb_idx][:, curr_ts_idx]
                mb_adv = advantages[curr_mb_idx][:, curr_ts_idx]
                mb_old_neglogp = - log_pi_taken[curr_mb_idx][:, curr_ts_idx]
                mb_mask = mask[curr_mb_idx][:, curr_ts_idx]
                mb_entropy_mask = entropy_mask[curr_mb_idx][:, curr_ts_idx]
                ...
                pi_taken = th.gather(pi, dim=3, index=actions).squeeze(3)
                pi_taken[mask == 0] = 1e-10
                mb_pi_taken = pi_taken[curr_mb_idx][:, curr_ts_idx]

                mb_log_pi_taken = th.log(mb_pi_taken)
                mb_neglogp = - mb_log_pi_taken

                actor_mask = mb_mask.expand_as(mb_neglogp)

                with th.no_grad():
                    approxkl = 0.5 * ((mb_old_neglogp - mb_neglogp)**2 * actor_mask).mean()
                    approxkl_lst.append(approxkl)

                entropy_matrix = 1.0 * (mb_pi_taken * mb_neglogp).sum(dim=-1)
                entropy = th.sum(entropy_matrix * mb_entropy_mask)/ ( mb_entropy_mask.sum() + 1e-5)
                ...
                pg_loss_lst.append(pg_loss)

                # Construct overall loss
                loss = pg_loss - self.args.entropy_loss_coeff * entropy
