...
class ParallelRunner:
            ...
            self.ps = [
                Process(
                    ...
                    args=(
                        worker_conn,
                        CloudpickleWrapper(
                            partial(env_fn, env_args=self.args.env_args, args=self.args)
                        ),
                    ),
                )
                for worker_conn in self.worker_conns
    ...
    def set_hidden_data(self, transition_data, num_agents, agents_alive=None):
        ...
        if hasattr(self.mac.agent, "h_in"):
            ...
            if agents_alive is None:
                ...
                transition_data["hidden"] = hidden[
                    agents_alive
                ]  # save for agents alive
    ...
    def run(self, test_mode=False):
        ...
        while True:
            ...
            # Save rnn hidden state if used
            self.set_hidden_data(
                pre_transition_data, actions.shape[1], agents_alive=envs_not_terminated
            )
