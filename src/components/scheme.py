...
class Scheme():
    ...
    # def join(self, schemes):
    #     """
    #     joins scheme with a bunch of other schemes
    #     """
    #
    #     # create a current scheme dictionary by rename/name
    #     self_dic = {}
    #     for _scheme_entry in self.scheme_list:
    #         self_dic[_scheme_entry.get("rename", _scheme_entry.get("name"))] = _scheme_entry
    #     self_keys_set = set(self_dic.keys())
    #
    #     for _scheme in schemes:
    #         # create a scheme key list
    #         _scheme_dic = {}
    #         for _scheme_entry in _scheme.scheme_list:
    #             _scheme_dic[_scheme_entry.get("rename", _scheme_entry.get("name"))] = _scheme_entry
    #         _scheme_key_set = set(_scheme_dic.keys())
    #
    #         # find set of scheme entries which are different
    #         disjoint_keys = _scheme_key_set.difference(self_keys_set)
    #         for _key in disjoint_keys:
    #             self.scheme_list.append(_scheme_dic[_key])
    #             self_dic[_key] = _scheme_dic[_key]
    #
    #         def _scheme_entry_equals(_a, _b):
    #             """
    #             this will not allow false positives, but may admit false negatives
    #             """
    #             if _a.get("transforms", None) != _b.get("transforms", None):
    #                 return False
    #             return True
    #
    #         # find set of scheme entries which are the same
    #         joint_keys = self_keys_set.intersection(_scheme_key_set)
    #         for _key in joint_keys: # check if overlapping keys have exactly the same attributes
    #             assert _scheme_entry_equals(_scheme_dic[_key], self_dic[_key]), "schemes are not congruent!"
    #             pass
    #
    #     return self
    # pass